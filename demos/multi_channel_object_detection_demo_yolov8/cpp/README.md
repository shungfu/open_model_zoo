# Multi-Channel Object Detection Yolov8 C++ Demo

This demo provides an multi-channel inference pipeline for YOLO v8 Object Detection network.
...
On startup, the application reads command line parameters and loads the specified networks. The Yolo v8 Object Detection network is required.
...
The list of models supported by the demo is in `<omz_dir>/demos/multi_channel_object_detection_demo_yolov8/cpp/models.lst` file.
...
```sh
./multi_channel_object_detection_demo_yolov8 -m <path_to_model>/model.xml -d CPU -i 0
```

To run the demo on HDDL, using two recorded video files, use the following command:

```sh
./multi_channel_object_detection_demo_yolov8 -m <path_to_mdel>/model.xml -d HDDL -i <path_to_file>/file1,<path_to_file>/file2
```
...
```sh
./multi_channel_object_detection_demo_yolov8 -m <path_to_model>/model.xml -d HDDL
